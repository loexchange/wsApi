...
from loex.constant.definition import *
...
def check_symbol(symbol):
    if not isinstance(symbol, str):
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] symbol must be string")
    if re.match(reg_ex, symbol):
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + symbol + "  is invalid symbol")
...
def check_time_in_force(time_in_force, order_type):
    ...
    if order_type in [OrderType.BUY_MARKET, OrderType.SELL_MARKET] \
        ...
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] timeInForce not supported for market order")


def check_symbol_list(symbols):
    if not isinstance(symbols, list):
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] symbols in subscription is not a list")
...
def check_currency(currency):
    if not isinstance(currency, str):
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] currency must be string")
    if re.match(reg_ex, currency) is not None:
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + currency + "  is invalid currency")
...
def check_range(value, min_value, max_value, name):
    ...
    if min_value > value or value > max_value:
        raise LoexApiException(LoexApiException.INPUT_ERROR,
...
def check_should_not_none(value, name):
    if value is None:
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + name + " should not be null")


def check_should_none(value, name):
    if value is not None:
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + name + " should be null")
...
def format_date(value, name):
    ...
    if not isinstance(value, str):
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + name + " must be string")
    try:
        new_time = time.strptime(value, "%Y-%m-%d")
        return time.strftime("%Y-%m-%d", new_time)
    except:
        raise LoexApiException(LoexApiException.INPUT_ERROR, "[Input] " + name + " is not invalid date format")
