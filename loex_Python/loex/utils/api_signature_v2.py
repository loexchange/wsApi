...
def create_signature_v2(api_key, secret_key, method, url, builder):
    if api_key is None or secret_key is None or api_key == "" or secret_key == "":
        raise LoexApiException(LoexApiException.KEY_MISSING,  "API key and secret key are required")

    timestamp = utc_now()
    builder.put_url("apiKey", api_key)
    builder.put_url("signatureVersion", "1.0")
    ...
    dig = hmac.new(secret_key.encode('utf-8'), msg=qs0.encode('utf-8'), digestmod=hashlib.sha256).digest()
    # 进行base64编码
