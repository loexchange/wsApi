...
from loex.utils.etf_result import etf_result_check
from loex.utils import *
import time

from loex.utils.print_mix_object import TypeCheck
    ...
    if status and len(status):
        if TypeCheck.is_basic(status): # for normal case
            if status == "error":
                ...
                raise LoexApiException(LoexApiException.EXEC_ERROR,
                                        "[Executing] " + str(err_code) + ": " + err_msg)
            elif status != "ok":
                raise LoexApiException(LoexApiException.RUNTIME_ERROR,
                                        "[Invoking] Response is not expected: " + status)
        elif TypeCheck.is_dict(status): # for https://status.loexgroup.com/api/v2/summary.json in example example/generic/get_system_status.py
        ...
        if bool(success) is False:
            ...
            err_msg = dict_data.get("message", "")
            if err_code == "":
                raise LoexApiException(LoexApiException.EXEC_ERROR, "[Executing] " + err_msg)
            else:
                raise LoexApiException(LoexApiException.EXEC_ERROR, "[Executing] " + str(err_code) + ": " + err_msg)
    else:
        raise LoexApiException(LoexApiException.RUNTIME_ERROR, "[Invoking] Status cannot be found in response.")
