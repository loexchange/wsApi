...
from loex.connection.impl.private_def import ConnectionState
...
class WebsocketManage:
    ...
    def __init__(self, api_key, secret_key, uri, request):
        self.__thread = None
        self.__trading_url = LOEX_WEBSOCKET_URI_PRO + "/ws"
        self.request = request
        self.reconnect_at = 0
        self.original_connection = None
        self.last_receive_time = 0
        self.logger = logging.getLogger("loex-client")
    ...
    def on_open(self, original_connection):
        ...
        if self.request.is_trading:
            try:
                if self.request.api_version == ApiVersion.VERSION_V2:
    ...
    def on_error(self, error_message):
        if self.request.error_handler is not None:
            exception = LoexApiException(LoexApiException.SUBSCRIPTION_ERROR, error_message)
